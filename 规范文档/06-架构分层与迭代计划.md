...
- **超时必有**：外部 HTTP 经 Settings 配置（默认：`HTTP_TIMEOUT_WEB=8`、`HTTP_TIMEOUT_RESEARCH=6`、`LLM_TIMEOUT_SECONDS=90`），禁止无限等待。
...
- **并发闸门**：`MAX_CONCURRENT_CHATS`（默认 4）；超额 `POST /api/chat` 返回 **429**，避免拖死事件循环。
...
| 向量 embedding（P3） | 默认关（`RAG_VECTOR_ENABLED=false`）；开启时用 `hash` 本地哈希向量或 `openai` 远程 API；索引落盘 `data/vector_index`；失败自动回退 TF-IDF |
...
| `RAG_EMBEDDING_MODEL` | `text-embedding-3-small` | 仅 `openai` provider 使用 |
...
2. **开启**：按文档分块建/增量重建本地索引；`retrieval=vector_hybrid`；Trace `retrieve_score.backend=vector`。
